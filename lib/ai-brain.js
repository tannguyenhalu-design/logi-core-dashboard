...
export async function extractInsightsFromChat({ message, reply, userName, projectsList }) {
  try {
    ...
    const extractPrompt = `Bạn là hệ thống trích xuất kiến thức từ cuộc hội thoại giữa Manager (Đại Ca) và AI Agent (Tiểu Đệ SD3) của công ty GHN Điện Máy B2B.

Cuộc hội thoại vừa xảy ra:
Manager hỏi: "${message}"
AI trả lời: "${reply}"
Người dùng: ${userName || "Manager"}
Dự án trong hệ thống: ${projectNames}

Nhiệm vụ: Phân tích và trích xuất TỐI ĐA 3 insights có giá trị từ cuộc hội thoại này (nếu có). Chỉ trích xuất nếu THỰC SỰ có thông tin hữu ích để học.

Trả về JSON array, mỗi phần tử có dạng:
{
  "type": "user_preference|business_insight|correction|faq|pattern",
  "topic": "chủ đề ngắn gọn (tối đa 5 từ)",
  "insight": "nội dung insight cụ thể, ngắn gọn (1-2 câu)",
  "confidence": 0.0-1.0
}

Quy tắc:
- user_preference: Manager quan tâm đến điều gì, hỏi theo cách nào, thích format gì
- business_insight: Pattern kinh doanh, dự án nào đang hot/có vấn đề
- correction: Nếu Manager chỉnh sửa thông tin sai của AI
- faq: Câu hỏi hay gặp mà AI trả lời tốt
- pattern: Pattern thời vụ, hành vi lặp lại
...
export async function generateWeeklySummary() {
  try {
    ...
    const resp = await ai.models.generateContent({
      ...
      contents: `Bạn là trợ lý tổng hợp kiến thức. Dưới đây là ${rows.length} insights AI Agent SD3 đã học được. 
