...
export async function getAllUsers() {
  ...
  return dataRows.map((row) => rowToUser(headers, row)).filter((u) => u.name || u.employeeId || u.email);
}
...
/**
 * TEMPORARY stopgap while GHN SSO client_id/secret aren't set up yet —
 * passwordless login for any @ghn.vn email, same as before SSO existed.
 * Keyed by Name (= the email itself) so it reuses the exact same
 * approval flow as SSO users; remove this + pages/api/auth.js and the
 * login form once SSO is live. See pages/api/auth.js.
 */
export async function resolvePasswordlessUser(email) {
  const key = String(email).trim().toLowerCase();
  const existing = await findUserByName(key);
  ...
  await sheets.spreadsheets.values.append({
    ...
    valueInputOption: "USER_ENTERED",
    resource: {
      values: [[key, "", "", "pending", "", "", now, now, "temp-email-signup", "", key, ""]],
    },
  });
 ...
 */
export async function createUserWithRole(name, { role, pic, project, tabs, updatedBy }) {
  const sheets = await getSheetsClient();
  await ensureUsersSheet(sheets);
  const now = new Date().toISOString();
  ...
  await sheets.spreadsheets.values.append({
    ...
    valueInputOption: "USER_ENTERED",
    resource: {
      values: [["", "", "", role, pic || "", project || "", now, now, updatedBy || "", tabsValue, String(name).trim(), ""]],
    },
  });
}
