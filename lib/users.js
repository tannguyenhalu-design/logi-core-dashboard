...
export async function createPendingUser(email, password) {
  ...
  // Specific emails get ops_specialist by default, others get pending
  const opsEmails = ["datnt2@ghn.vn", "tutd@ghn.vn", "diennk@ghn.vn"];
  const defaultRole = opsEmails.includes(emailLower) ? "ops_specialist" : "pending";
  ...
  await sheets.spreadsheets.values.append({
    ...
    valueInputOption: "USER_ENTERED",
    resource: {
      values: [[emailLower, hash, salt, defaultRole, "", "", now, now, "self-signup", ""]],
    },
  });
}
