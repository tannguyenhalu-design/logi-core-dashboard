...
const HEADERS = ["Email", "PasswordHash", "Salt", "Role", "PIC Name", "Project", "CreatedAt", "UpdatedAt", "UpdatedBy"];
...
export async function createPendingUser(email, password) {
  ...
  await sheets.spreadsheets.values.append({
    ...
    valueInputOption: "USER_ENTERED",
    resource: {
      values: [[String(email).trim().toLowerCase(), hash, salt, "pending", "", "", now, now, "self-signup"]],
    },
  });
}
 ...
 */
export async function createUserWithRole(email, { role, pic, project, updatedBy }) {
  const sheets = await getSheetsClient();
  await ensureUsersSheet(sheets);
  const now = new Date().toISOString();
  await sheets.spreadsheets.values.append({
    ...
    valueInputOption: "USER_ENTERED",
    resource: {
      values: [[String(email).trim().toLowerCase(), "", "", role, pic || "", project || "", now, now, updatedBy || ""]],
    },
  });
}
...
export async function updateUserRole(email, { role, pic, project, updatedBy }) {
  const sheets = await getSheetsClient();
  const { headers, dataRows } = await getRows(sheets);
  const emailIdx = headers.indexOf("Email");
