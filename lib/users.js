...
export const ALL_TABS = ["ltl", "operations", "tachtrip", "ftl"];
...
export async function deleteUser({ employeeId, name }) {
  ...
  // Delete the row itself (shifts everything below up), not just its cell
  // contents. clear() left a fully-blank row in place — a real incident
  // (2026-08-17): with blank rows sitting mid-table, Sheets' values.append()
  // (used by createUserWithRole/resolveSSOUser to add new accounts)
  // misjudged where the table ends and wrote the next new user's row
  // starting several columns over, silently corrupting it (columns landed
  // under the wrong headers, EmployeeId never got set, so the account was
  // both broken AND unfindable by any of the lookup functions above).
  const meta = await sheets.spreadsheets.get({ spreadsheetId: getSpreadsheetId() });
...
/**
 * lookup: { employeeId } once linked, or { name } for a not-yet-logged-in
 * provisioned row — must describe the row's CURRENT identity, exactly as
 * it already exists in the sheet, never a value being set for the first
 * time by this same call. updates: the new field values to write,
 * INCLUDING employeeId/name if those are what's changing (e.g. a manager
 * typing in a not-yet-linked person's EmployeeId to approve them).
 *
 * These two used to be the same {employeeId, name} pair reused for both
 * purposes — real incident (2026-08-17): approving a freshly self-registered
 * "pending" account (found by name, since it had no EmployeeId yet) passed
 * the manager's newly-typed EmployeeId as the LOOKUP key too. The row's
 * EmployeeId column was still blank at that point (this call was supposed
 * to be the one that sets it), so the lookup searched for a value that
 * didn't exist anywhere yet and threw "Không tìm thấy tài khoản này" —
 * looking the row up by the very value it was trying to assign.
 */
export async function updateUserRole({ employeeId: lookupEmployeeId, name: lookupName }, { employeeId: newEmployeeId, name: newName, role, pic, project, tabs, updatedBy }) {
  const sheets = await getSheetsClient();
  const { headers, dataRows } = await getRows(sheets);
  const employeeIdx = headers.indexOf("EmployeeId");
  const nameIdx = headers.indexOf("Name");
  ...
  const targetEmployeeId = String(lookupEmployeeId || "").trim();
  ...
  if (newName && nameIdx !== -1) updates.push([nameIdx, newName]);
  if (newEmployeeId && employeeIdx !== -1) updates.push([employeeIdx, newEmployeeId]);
