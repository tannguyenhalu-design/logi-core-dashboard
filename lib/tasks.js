...
const HEADERS = ["ID", "Title", "PIC", "PICName", "Project", "Deadline", "Status", "Notes", "CreatedAt", "CreatedBy", "UpdatedAt", "UpdatedBy", "CalendarEventId", "CalendarLink", "GroupId"];
  ...
  await sheets.spreadsheets.values.append({
    ...
    resource: {
      values: created.map((t) => [
        ...
        t.createdAt, t.createdBy, t.updatedAt, t.updatedBy, t.calendarEventId, t.calendarLink, t.groupId,
      ]),
    },
  });
...
export async function updateTaskStatus(id, status, actor) {
  ...
  return rowToTask([...rows[rowIdx].slice(0, 6), status, rows[rowIdx][7], rows[rowIdx][8], rows[rowIdx][9], now, actor, rows[rowIdx][12], rows[rowIdx][13], rows[rowIdx][14]]);
}
