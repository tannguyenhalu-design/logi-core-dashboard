...
const HEADERS = ["ID", "Title", "PIC", "PICName", "Project", "Deadline", "Status", "Notes", "CreatedAt", "CreatedBy", "UpdatedAt", "UpdatedBy"];
...
export async function createTasks(tasksInput, actor) {
  ...
  const created = tasksInput.map((t, i) => ({
    id: `task-${Date.now()}-${i}`,
    title: t.title,
    pic: t.pic,
...
export async function updateTaskStatus(id, status, actor) {
  ...
  return rowToTask([...rows[rowIdx].slice(0, 6), status, rows[rowIdx][7], rows[rowIdx][8], rows[rowIdx][9], now, actor]);
}
