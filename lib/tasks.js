...
// deliberately untouched here; those stay per-person via updateTaskStatus.
export async function updateTaskDetails(groupId, { title, project, deadline, notes }, actor) {
  const sheets = await getSheetsClient();
  await ensureSheet(sheets);
  const resp = await sheets.spreadsheets.values.get({
    spreadsheetId: SPREADSHEET_ID,
  ...
  // Best-effort: keep each assignee's calendar event in sync (new title/
  // deadline) — a failure here must not fail the edit itself.
  await Promise.all(matches.map(async ({ r }) => {
    const calendarEventId = r[12];
    if (!calendarEventId) return;
    await updateTaskEvent(calendarEventId, {
      title: title !== undefined ? title : r[1],
      deadline: deadline !== undefined ? deadline : r[5],
      pic: r[2],
      picName: r[3],
      project: project !== undefined ? project : r[4],
      notes: notes !== undefined ? notes : r[7],
    });
  }));

  return matches.map(({ r, i }) => rowToTask([
    r[0],
    title !== undefined ? title : r[1],
    r[2], r[3],
    project !== undefined ? project : r[4],
    deadline !== undefined ? deadline : r[5],
    r[6],
    notes !== undefined ? notes : r[7],
