...
export function transformLTL(rawRows, filters = {}, rawDamage = []) {
  ...
  // Group key follows the same date field as filterMode (delivered_time in
  // GTC mode), matching ordersByMonth — otherwise an order delivered this
  // month but picked up in an earlier month gets miscounted into that
  // earlier month here while everywhere else counts it in this month.
  const ontimeByMonth = {};
  deliveredRows.forEach((r) => {
