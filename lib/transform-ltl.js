...
export function transformLTL(rawRows, filters = {}, rawDamage = []) {
  ...
  // ── Pickup-point (origin) breakdown — mirror image of the above, grouped
  // by from_province_name instead. Lets a single-project view show
  // "performance FROM this specific pickup point" (e.g. a client that ships
  // out of both Bình Dương and Hà Nội warehouses) instead of only
  // "performance TO this destination across every pickup point combined".
  const { stats: originStats, detailsMap: originDetailsMapBase } =
    buildLocationBreakdown(rows, filteredDamage, "from_province_name", "to_province_name");

  // For each origin, also build its OWN destination-side breakdown (same
  // shape as provinceStats/provinceDetailsMap above, just scoped to rows
  // picked up from that one origin) — this is what lets the map/hover-card/
  // "Top 8 Tỉnh" UI swap to "filtered by this pickup point" using the exact
  // same rendering code, just fed a different data source.
  const originDetailsMap = {};
  Object.keys(originDetailsMapBase).forEach((origin) => {
    const originRows = rows.filter((r) => String(r["from_province_name"] || "").trim() === origin);
    const originDamage = filteredDamage.filter((r) => String(r["from_province_name"] || "").trim() === origin);
    const { stats: destStats, detailsMap: destDetailsMap } =
      buildLocationBreakdown(originRows, originDamage, "to_province_name", "from_province_name");
    originDetailsMap[origin] = {
      ...originDetailsMapBase[origin],
      provinceStats: destStats,
      provinceDetailsMap: destDetailsMap,
    };
  });
...
export function computePeriodComparison(rawRows, periodMode = "mtd") {
  ...
  function statsFor(start, end, getValue, value) {
    ...
    // Raw "weight" column is in grams — /1000 to match kg everywhere else
    // in this file (totalWeight, weightByProject, etc.).
    const weight = inRange.reduce((sum, r) => sum + ((parseFloat(r["weight"]) || 0) / 1000), 0);
    return {
  ...
  // prev === 0 makes the % ratio undefined (chia cho 0), but that's a
  // real, well-defined signal — "mới phát sinh" — not a "chưa có dữ liệu"
  // case, so it's flagged separately (isNew) instead of just going null.
