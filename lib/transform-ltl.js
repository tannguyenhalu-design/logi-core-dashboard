...
export function computeConsecutiveDeclineAlerts(rawRows) {
  ...
  Object.entries(byClient).forEach(([client, buckets]) => {
    ...
    const series = [];
    for (let b = DECLINE_WEEKS_LOOKBACK; b >= 1; b--) series.push(buckets[b] || 0);
    ...
    alerts.push({ client, weeklyCounts: lastN, weeksDeclining: DECLINE_WEEKS_REQUIRED, totalDeclinePct });
  });
