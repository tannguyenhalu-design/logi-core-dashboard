...
export function transformLTL(rawRows, filters = {}, rawDamage = []) {
  ...
  return {
    ...
    routeStats,
    filteredRows: rows,
    // Always computed off all rows (not the month/project filters above) —
    // this is a "what changed recently" panel, not a filtered view.
    periodComparison: computePeriodComparison(rawRows),
  };
}

const PERIOD_WINDOW_DAYS = 7; // so sánh 7 ngày gần nhất vs 7 ngày trước đó
...
/**
 * So sánh cùng kỳ — rolling 7-ngày gần nhất vs 7-ngày trước đó (thay vì
 * so tháng): so tháng nghe hợp lý nhưng lệch, vì tháng đang chạy chỉ có
 * vài ngày dữ liệu (đa số đơn chưa kịp bị đánh dấu trễ), trong khi
 * tháng trước đã có cả tháng để "chín" — so sánh kiểu đó luôn ra tháng
 * này "đẹp" hơn một cách giả tạo, đúng cái làm dashboard "đẹp quá" ban
 * đầu. Cửa sổ 7 ngày (lùi lại 2 ngày đệm cho đơn kịp có kết quả) tránh
 * được cả lệch tháng lẫn lệch độ chín, và không cần chờ có đủ 2 tháng
 * dữ liệu lịch sử mới dùng được.
 */
export function computePeriodComparison(rawRows) {
  const rows = Array.isArray(rawRows) ? rawRows : [];
  const now = new Date();
  const today0 = new Date(now.getFullYear(), now.getMonth(), now.getDate());

  const curEnd = new Date(today0);
  curEnd.setDate(curEnd.getDate() - PERIOD_BUFFER_DAYS);
  curEnd.setHours(23, 59, 59, 999);
  const curStart = new Date(curEnd);
  curStart.setDate(curStart.getDate() - PERIOD_WINDOW_DAYS + 1);
  ...
  prevStart.setDate(prevStart.getDate() - PERIOD_WINDOW_DAYS + 1);
  prevStart.setHours(0, 0, 0, 0);

  const fmtRange = (s, e) =>
    `${String(s.getDate()).padStart(2, "0")}/${String(s.getMonth() + 1).padStart(2, "0")} - ${String(e.getDate()).padStart(2, "0")}/${String(e.getMonth() + 1).padStart(2, "0")}`;

  function statsFor(start, end, clientName) {
    const inRange = rows.filter((r) => {
      if (clientName && String(r["client_name"] || "").trim() !== clientName) return false;
  ...
  const clients = clientNames
    .map((name) => {
      const cur = statsFor(curStart, curEnd, name);
      const prev = statsFor(prevStart, prevEnd, name);
      ...
      return { client: name, cur, prev, ontimeDeltaPoints, ordersDeltaPct };
    })
  ...
  return {
    currentRangeLabel: fmtRange(curStart, curEnd),
