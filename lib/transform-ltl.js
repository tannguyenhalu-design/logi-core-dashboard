...
export function transformLTL(rawRows, filters = {}, rawDamage = []) {
  ...
  // ── Province distribution (for the LTL map + AI insight panel) ──
  // Uses `rows` (already filtered by month/project/filterMode above), so
  // selecting a single project naturally scopes this to just that project.
  const provinceMap = {};
  rows.forEach((r) => {
