...
export function transformLTL(rawRows, filters = {}) {
  ...
  // Apply filters — main filter uses pickup_time per spec
  ...
  // GTC theo delivered_time (orders giao xong trong tháng đó, bất kể pickup tháng nào)
  const deliveredThisMonthCount = months && months.length > 0
    ? rawRows.filter(row => {
        if (projects && projects.length > 0 && !projects.includes(row["client_name"])) return false;
  ...
  // Orders by month (or by week if isWeekly is true)
  const ordersByMonth = {};
  const weightByMonth = {};
  rows.forEach((r) => {
