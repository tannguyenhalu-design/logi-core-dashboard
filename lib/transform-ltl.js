...
export function transformLTL(rawRows, filters = {}, rawDamage = []) {
  ...
  // raw_damage's own to_province_name/from_province_name columns are always
  // empty in the real sheet (GHN's damage feed never populates them) — so
  // every province-level damage grouping (the map view, per-province and
  // per-client damage counts) always summed to 0, independent of any
  // project filter. Backfill them from the matching order in raw_ontime via
  // order_code, since that's a 1:1 join and raw_ontime always has both
  // province fields.
  const provinceByOrderCode = new Map();
  (rawRows || []).forEach((r) => {
    const code = String(r["order_code"] || "").trim();
    if (code && !provinceByOrderCode.has(code)) {
      provinceByOrderCode.set(code, {
        to: r["to_province_name"] || "",
        from: r["from_province_name"] || "",
        clientName: r["client_name"] || "",
      });
    }
  });
  rawDamage = (rawDamage || []).map((r) => {
    const match = provinceByOrderCode.get(String(r["order_code"] || "").trim());
    if (!match) return r;
    // Rillnet's own client_name for a case can be the client's bare/legacy
    // name ("PSD") while raw_ontime's real order uses a region-split name
    // ("PSD Miền Nam"/"PSD Miền Trung") — confirmed live: 6 real PSD damage
    // cases all recorded under bare "PSD" in raw_damage_causes, which
    // doesn't match ANY real client_name in raw_ontime, so they formed
    // their own disconnected 0-order "PSD" bucket in every per-client
    // breakdown instead of counting against the real "PSD Miền Nam"/"PSD
    // Miền Trung" — making it look like PSD's damage count "appeared from
    // nowhere" instead of being attributed to the actual client. The order
    // itself (joined by order_code, same 1:1 join as the province backfill
    // above) always has the correct, real client_name — prefer it.
    return {
      ...r,
      to_province_name: r["to_province_name"] || match.to,
      from_province_name: r["from_province_name"] || match.from,
      client_name: match.clientName || r["client_name"],
    };
  });
  ...
  // raw_damage's own client_name spelling doesn't always match the project
  // list's casing (e.g. project "AQUA B2C" vs raw_damage's "Aqua B2C") —
  // an exact `projects.includes(clientName)` silently matched nothing for
  // several clients, so every damage stat quietly zeroed out the moment a
  // single project was selected while the unfiltered "all projects" view
  // still showed real numbers (no project filter to trip over). Case-
  // insensitive equality first, then a substring fallback in either
  // direction for split sub-brands raw_damage doesn't distinguish (e.g.
  // "Casper" covers both "Casper B2B" and "Casper B2C" projects).
  ...
  // Orders by project, broken down by week-of-month — only meaningful when
  // a single month is selected (isWeekly). Lets a user (or the AI chat)
  // answer "which client caused week 2 → week 3 to drop" directly instead
  // of only seeing the combined weekly total (which was all the trend
  // chart showed before — real question, no way to self-serve the answer).
  const ordersByProjectAndWeek = {};
  if (isWeekly) {
    rows.forEach((r) => {
      const proj = r["client_name"] || "Unknown";
      const dateField = filterMode === "delivered" ? r["delivered_time"] : r["pickup_time"];
      const week = getWeekFromPickupTime(dateField);
      if (!week) return;
      if (!ordersByProjectAndWeek[proj]) ordersByProjectAndWeek[proj] = {};
      ordersByProjectAndWeek[proj][week] = (ordersByProjectAndWeek[proj][week] || 0) + 1;
    });
  }
  ...
  return {
    ...
    periodComparison: computePeriodComparison(
      ...
      filteredDamageForPicker.filter((r) => passOriginDmg(r["from_province_name"]))
    ),
    declineAlerts: computeConsecutiveDeclineAlerts(rawRows.filter((r) => passProject(r) && passOrigin(r))),
  };
}
 ...
 */
export function computePeriodComparison(rawRows, periodMode = "mtd", minSample = PERIOD_MIN_SAMPLE, rawDamage = []) {
  ...
  const curAll = { orders: 0, evalCount: 0, ontimeCount: 0, weight: 0, damageCount: 0 };
  const prevAll = { orders: 0, evalCount: 0, ontimeCount: 0, weight: 0, damageCount: 0 };
  const curByClient = {};
  const prevByClient = {};
  const curByWarehouse = {};
  const prevByWarehouse = {};

  const initStats = () => ({ orders: 0, evalCount: 0, ontimeCount: 0, weight: 0, damageCount: 0 });
  ...
  // ── Count damage cases per period ──
  const damageRows = Array.isArray(rawDamage) ? rawDamage : [];
  damageRows.forEach((r) => {
    const dateVal = r["case_date"] || r["pickup_time"];
    const d = parseDate(dateVal);
    if (!d) return;

    let isCur = d >= curStart && d <= curEnd;
    let isPrev = d >= prevStart && d <= prevEnd;
    if (!isCur && !isPrev) return;
    ...
    if (isCur) {
      curAll.damageCount++;
      if (client) {
        if (!curByClient[client]) curByClient[client] = initStats();
        curByClient[client].damageCount++;
      }
    } else {
      prevAll.damageCount++;
      if (client) {
        if (!prevByClient[client]) prevByClient[client] = initStats();
        prevByClient[client].damageCount++;
      }
    }
  });
  ...
  function breakdownByFast(curMap, prevMap) {
    ...
    return names
      .map((name) => {
        ...
        const { deltaPct: damageDeltaPct, isNew: damageIsNew } = pctDelta(cur.damageCount, prev.damageCount);
        return { name, cur, prev, ontimeDeltaPoints, ordersDeltaPct, ordersIsNew, weightDeltaPct, weightIsNew, damageDeltaPct, damageIsNew };
      })
      // A genuinely new client/warehouse (prev period had zero) always
      // passes regardless of order count — minSample exists to hide noisy
      // %-swings on tiny established accounts, but a first-ever appearance
      // isn't noise, it's exactly the signal this panel exists to surface.
      // Previously a brand-new client with < minSample orders in its first
      // period was dropped here entirely, so it never got flagged "🆕 Mới"
      // anywhere (dashboard card or AI narrative) until it crossed the
      // threshold — by which point it was old news.
      .filter((c) => c.cur.orders >= minSample || c.prev.orders >= minSample || c.ordersIsNew || c.damageIsNew)
      .map((c) => ({
        ...c,
        warning: (c.ontimeDeltaPoints != null && c.ontimeDeltaPoints <= -PERIOD_ONTIME_WARN_POINTS) ||
                 ...
                 // A client with a real, sizable damage count (or a fresh
                 // damage increase) is exactly what this panel is for —
                 // previously only ontime/order drops triggered ⚠️, so a
                 // client with stable ontime% but a serious breakage
                 // problem (confirmed live: Aqua B2C 46 cases, LG LTL 23
                 // cases) never got flagged here at all.
                 (c.cur.damageCount >= 3) || (c.damageIsNew && c.cur.damageCount > 0),
      }))
      // New entries and damage-significant clients bubble to the front so
      // they aren't crowded out by clients with a worse ontime delta.
      .sort((a, b) => {
        if (a.ordersIsNew !== b.ordersIsNew) return a.ordersIsNew ? -1 : 1;
        if ((a.cur.damageCount > 0) !== (b.cur.damageCount > 0)) return a.cur.damageCount > 0 ? -1 : 1;
        return (a.ontimeDeltaPoints ?? 0) - (b.ontimeDeltaPoints ?? 0);
      });
    // NOTE: deliberately NOT sliced here anymore — this used to cap at 12,
    // which silently dropped high-damage-count clients whose ontime% was
    // stable (they sorted to the bottom by ontime delta and never survived
    // the cut) from every downstream consumer: the AI narrative's
    // damageByProject list, newOrReturningClients, growingClients — not
    // just the on-screen card grid. The card grid now applies its own
    // display cap at render time (components/TabAIInsights.js) so the UI
    // stays uncluttered while the underlying data stays complete.
  }
  ...
  const overallDamage = pctDelta(overallCur.damageCount, overallPrev.damageCount);
  ...
  return {
    ...
    overall: {
      ...
      damageDeltaPct: overallDamage.deltaPct,
      damageIsNew: overallDamage.isNew,
    },
...
const DECLINE_WEEKS_REQUIRED = 3; // cần 3 tuần lịch liên tiếp đều giảm mới báo
const DECLINE_WEEKS_LOOKBACK = 4; // quét trong 4 tuần gần nhất (bỏ tuần hiện tại)
const DECLINE_MIN_SAMPLE = 5;     // tuần đỉnh phải có ít nhất 5 đơn, tránh nhiễu khách quá nhỏ

/**
 * Cảnh báo giảm nhiều kỳ liên tiếp — khác với periodComparison ở trên (chỉ
 * so kỳ này với đúng kỳ đó THÁNG TRƯỚC, 2 điểm dữ liệu), cái này dùng 4 TUẦN
 * LỊCH LIÊN TIẾP gần nhất, không phụ thuộc ranh giới tháng, để bắt xu hướng
 * giảm thật: 1 kỳ giảm có thể ngẫu nhiên, giảm liên tục nhiều tuần mới là
 * tín hiệu cần ưu tiên kiểm tra. Tuần hiện tại (chưa hết 7 ngày) bị loại ra
 * vì số đơn của nó luôn thấp hơn giả tạo so với các tuần đã kết thúc.
 */
export function computeConsecutiveDeclineAlerts(rawRows) {
  const rows = Array.isArray(rawRows) ? rawRows : [];
  const now = new Date();
  const today0 = new Date(now.getFullYear(), now.getMonth(), now.getDate());

  // bucket 0 = tuần hiện tại (loại), 1 = tuần đã kết thúc gần nhất, 2 = tuần
  // trước đó nữa, v.v. — tính bằng số ngày lùi từ hôm nay, chia khối 7 ngày.
  const bucketFor = (d) => {
    const d0 = new Date(d.getFullYear(), d.getMonth(), d.getDate());
    const diffDays = Math.floor((today0 - d0) / 86400000);
    if (diffDays < 0) return null;
    return Math.floor(diffDays / 7);
  };

  // Ngày cụ thể của khối tuần "b", để hiện nhãn dd/mm-dd/mm thay vì chỉ số
  // trần trụi — bucket b phủ [today0 - (b*7+6) ngày, today0 - b*7 ngày].
  ...
  rows.forEach((r) => {
    ...
    if (bucket === null || bucket === 0 || bucket > DECLINE_WEEKS_LOOKBACK) return;
    const client = String(r["client_name"] || "").trim();
    if (!client) return;
    if (!byClient[client]) byClient[client] = {};
    byClient[client][bucket] = (byClient[client][bucket] || 0) + 1;
  });

  const alerts = [];
  Object.entries(byClient).forEach(([client, buckets]) => {
    // Xếp theo thời gian: cũ nhất → gần nhất (bucket lớn nhất trước).
    const bucketOrder = [];
    for (let b = DECLINE_WEEKS_LOOKBACK; b >= 1; b--) bucketOrder.push(b);
    const series = bucketOrder.map((b) => buckets[b] || 0);
    const lastBuckets = bucketOrder.slice(-DECLINE_WEEKS_REQUIRED);
    const lastN = series.slice(-DECLINE_WEEKS_REQUIRED);
    if (lastN.length < DECLINE_WEEKS_REQUIRED) return;
    const isDeclining = lastN.every((v, i) => i === 0 || v < lastN[i - 1]);
    if (!isDeclining || lastN[0] < DECLINE_MIN_SAMPLE) return;
    const totalDeclinePct = Math.round(((lastN[lastN.length - 1] - lastN[0]) / lastN[0]) * 100);
    const weeklyBreakdown = lastBuckets.map((b, i) => ({ label: labelForBucket(b), count: lastN[i] }));
    alerts.push({ client, weeklyBreakdown, weeksDeclining: DECLINE_WEEKS_REQUIRED, totalDeclinePct });
  });

  return alerts.sort((a, b) => a.totalDeclinePct - b.totalDeclinePct);
}
