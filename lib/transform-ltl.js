...
export function transformLTL(rawRows, filters = {}, rawDamage = []) {
  const { months = null, projects = null, filterMode = "pickup", periodWeeks = 1 } = filters;
...
/**
 * So sánh cùng kỳ — khối ngày-trong-tháng (VD tuần 1 = 1-7, windowWeeks=1)
 * của tháng này vs cùng khối đó của tháng trước, thay vì so cả tháng: so cả
 * tháng nghe hợp lý nhưng lệch, vì tháng đang chạy chỉ có vài ngày dữ liệu
 * (đa số đơn chưa kịp bị đánh dấu trễ) trong khi tháng trước đã có cả tháng
 * để "chín" — so kiểu đó luôn ra tháng này "đẹp" hơn giả tạo.
 *
 * Để không lặp lại lỗi đó ở cấp tuần: nếu khối hiện tại (chứa hôm nay) CHƯA
 * kết thúc (trừ hao buffer ngày), lùi về khối liền trước — luôn so 2 khối
 * đã khép, không bao giờ so khối đang chạy dở với khối đã xong.
 */
export function computePeriodComparison(rawRows, windowWeeks = 1) {
  const rows = Array.isArray(rawRows) ? rawRows : [];
  const weeks = [1, 2, 3].includes(windowWeeks) ? windowWeeks : 1;
  const now = new Date();
  ...
  const curStart = curBlock.start, curEnd = curBlock.end;
  const prevStart = prevBlock.start, prevEnd = prevBlock.end;
  ...
  return {
    windowWeeks: weeks,
    currentRangeLabel: fmtRange(curBlock),
    previousRangeLabel: fmtRange(prevBlock),
    overall: {
      cur: overallCur,
      prev: overallPrev,
      ontimeDeltaPoints:
