 ...
 *   - kho_lay             : kho xuất phát (hub)
 *   - kho_giao            : kho lastmile (Lens 2)
...
function getKhoLay(r) { return r["kho_lay"] || r["warehouse_lay"]; }
function getKhoGiao(r) { return r["kho_giao"] || r["warehouse_giao"]; }
...
export function transformLaneData(rawRows, windowDays = 30) {
  ...
  rows.forEach((r) => {
    ...
    const pickWh  = String(getKhoLay(r)  || "").trim();
    const delivWh = String(getKhoGiao(r) || "").trim();
...
export function transformTachTrip(rawRows) {
  ...
  rows.forEach((r) => {
    ...
    hubSet.add(getHub(getKhoLay(r)));
    warehouseSet.add(String(getKhoGiao(r) || "").trim());
  });
  ...
  rows.forEach((r) => {
    ...
    const hub = getHub(getKhoLay(r));
    const wh  = String(getKhoGiao(r) || "").trim();
    const w   = parseFloat(r["weight"])        || 0;
