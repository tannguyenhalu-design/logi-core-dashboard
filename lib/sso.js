...
export async function verifyIdToken(idToken, expectedNonce) {
  // GHN's real tokens ship an empty "aud" claim, not the client_id
  // their own docs show — confirmed via decoded-claims logging against
  // production. So audience isn't checked here; the nonce (tied to our
  // own sealed cookie from sso-login.js) is what proves this token
  // belongs to the request we made, which covers the same CSRF/replay
  // concern audience validation would otherwise cover.
  let payload;
  try {
    ...
    try {
      console.error("[sso] jwtVerify failed:", err.code || err.message, "decoded claims:", JSON.stringify(decodeJwt(idToken)));
    } catch {}
    throw err;
  }
  if (payload.nonce !== expectedNonce) {
    console.error(
      "[sso] nonce mismatch — token nonce:", JSON.stringify(payload.nonce),
      "expected (from sso_flow cookie):", JSON.stringify(expectedNonce)
    );
