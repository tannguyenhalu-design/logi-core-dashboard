...
export async function verifyIdToken(idToken, expectedNonce) {
  ...
  try {
    ...
    // Diagnostic only — logs the unverified claims so a claim-shape
    // mismatch (e.g. GHN's aud not being exactly our client_id) is
    // visible in the logs instead of just "verification failed".
    try {
