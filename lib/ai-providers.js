 ...
 * 1. Gemini 2.0 Flash      — Google, 1500 req/day, thông minh nhất, 1M context
 * 2. Groq Llama 3.3 70B    — Groq, 14400 req/day, cực NHANH (~500 tok/s)
 * 3. Gemini 1.5 Flash      — Google, backup chắc chắn
 * 4. Groq Gemma 2 9B       — Groq, nhẹ hơn, last resort
 * 5. Gemini 1.5 Flash 8B   — Google, lightest
 */
...
const PROVIDERS = [
  {
    ...
    call: (args) => callGemini({ ...args, model: "gemini-2.0-flash" }),
    type: "gemini",
  },
  {
  ...
  },
  {
    ...
    call: (args) => callGemini({ ...args, model: "gemini-1.5-flash" }),
    type: "gemini",
  },
  {
  ...
  },
  {
    ...
    call: (args) => callGemini({ ...args, model: "gemini-1.5-flash-8b" }),
    type: "gemini",
  },
];
...
export async function generateFast({ systemPrompt, userPrompt, temperature = 0.1 }) {
  const fastProviders = [
    ...
    {
      name: "Gemini 1.5 Flash 8B",
      call: () => callGemini({ model: "gemini-1.5-flash-8b", systemPrompt, userPrompt, temperature }),
    },
    {
      name: "Gemini 1.5 Flash",
      call: () => callGemini({ model: "gemini-1.5-flash", systemPrompt, userPrompt, temperature }),
    },
  ];
