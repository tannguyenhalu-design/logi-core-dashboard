...
export function transformAIInsights(rawRows, rawDamage = [], periodWeeks = "mtd") {
  ...
  const periodComparison = computePeriodComparison(rows, periodWeeks, undefined, rawDamage);
...
// Root-cause breakdown for damage/breakage cases, sourced from Rillnet
// (rillnet-app.vercel.app) via scripts/rillnet_scraper.py — Rillnet tracks
// which leg of the journey each case was first flagged at ("chặng nghi
// vấn"), which raw_damage doesn't carry. rawCauses = rows from the
// "raw_damage_causes" sheet tab (see lib/rillnet-sync.js for the write side).
export function computeDamageCauseBreakdown(rawCauses) {
  if (!rawCauses || rawCauses.length === 0) {
    return { totalCases: 0, byLeg: [], byClient: [] };
  }

  const legCounts = {};
  const clientCounts = {};
  for (const r of rawCauses) {
    const leg = String(r["suspected_leg"] || "Không rõ").trim() || "Không rõ";
    const client = String(r["client_name"] || "Không rõ").trim() || "Không rõ";
    legCounts[leg] = (legCounts[leg] || 0) + 1;
    clientCounts[client] = (clientCounts[client] || 0) + 1;
  }

  const total = rawCauses.length;
  const toSortedPct = (counts) =>
    Object.entries(counts)
      .map(([label, count]) => ({ label, count, pct: +((count / total) * 100).toFixed(0) }))
      .sort((a, b) => b.count - a.count);

  return {
    totalCases: total,
    byLeg: toSortedPct(legCounts),
    byClient: toSortedPct(clientCounts),
  };
}
