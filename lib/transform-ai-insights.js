...
export function transformAIInsights(rawRows) {
  ...
  // rawRows là array of objects — mỗi row là { weight, status, warehouse_lay, ... }
  ...
  rows.forEach(row => {
    ...
    const lay    = String(row["warehouse_lay"]  || "").trim();
    const giao   = String(row["warehouse_giao"] || "").trim();
    if (!lay || !giao) return;

    const key      = `${lay} → ${giao}`;
    const status   = String(row["status"]         || "").toLowerCase().trim();
    const weight   = parseFloat(row["weight"])    || 0;
    const damage   = String(row["Tình trạng"]     || "").trim();
