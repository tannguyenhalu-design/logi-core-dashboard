...
function getWeekOfMonthFromDateStr(dateStr) {
...
function buildTripRecords(rawFtlRows, vehicleLookup) {
  ...
  Object.entries(byTrip).forEach(([tripCode, rows]) => {
    ...
    const month = getMonth(dateVal);
     ...
     // Find DELIVERY rows for province and locations
    ...
    const locations = [
      ...new Set(
        deliveryRows
          .map((r) => String(r["location_name"] || "").trim())
          .filter(Boolean)
      ),
    ];
    ...
    // Actual vehicle capacity from GHN (xe_ghn_cung_cap)
    const xe_ghn_cap = (() => {
      const raw = String(firstPickup["xe_ghn_cung_cap"] || "").trim();
      if (!raw) return "Không rõ";
      const n = Number(raw);
      if (isNaN(n)) return raw;
      const map = { 1250:"1.25T", 1900:"1T9", 2500:"2.5T", 3500:"3.5T",
                   5000:"5T", 7000:"7T", 8000:"8T", 10000:"10T",
                   11000:"11T", 12000:"12T", 15000:"15T" };
      return map[n] || `${n/1000}T`;
    })();
...
export function transformFTL(rawFtlRows, masterVehicleRows, filters = {}) {
  ...
  // Trips by month (or by week if isWeekly is true)
  const tripsByMonth = {};
  trips.forEach((t) => {
    if (isWeekly) {
      const w = getWeekOfMonthFromDateStr(t.date);
      if (w) tripsByMonth[w] = (tripsByMonth[w] || 0) + 1;
    } else {
      if (!t.month) return;
  ...
  // Per-project detail: trips, locations, avg locations/trip
  const projectStats = {};
  trips.forEach((t) => {
    if (!projectStats[t.client]) projectStats[t.client] = { trips: 0, totalLocations: 0 };
    projectStats[t.client].trips++;
    projectStats[t.client].totalLocations += (t.locations?.length || 0);
  });
  ...
  // Overall avg locations per trip
  const totalLocations = trips.reduce((s, t) => s + (t.locations?.length || 0), 0);
  const avgLocationsPerTrip = trips.length > 0 ? Math.round((totalLocations / trips.length) * 10) / 10 : 0;
  ...
  // Top 10 Delivery Locations (trips count)
  const tripsByLocation = {};
  trips.forEach((t) => {
    if (t.locations) {
      t.locations.forEach((loc) => {
        tripsByLocation[loc] = (tripsByLocation[loc] || 0) + 1;
      });
    }
  });
  const top10Locations = Object.entries(tripsByLocation)
    .map(([location, count]) => ({ location, count }))
    .sort((a, b) => b.count - a.count)
    .slice(0, 10);

  // Province surge alerts (spec 3.8) — dynamically filtered by month and project
  const allTripsForAlert = buildTripRecords(rawFtlRows, vehicleLookup);
  let filteredForAlert = allTripsForAlert;
  if (months && months.length > 0) {
    filteredForAlert = filteredForAlert.filter((t) => months.includes(t.month));
  }
  if (projects && projects.length > 0) {
    filteredForAlert = filteredForAlert.filter((t) => projects.includes(t.client));
  }
  ...
  // All unique vehicle types from filtered trips
  const allVehicleTypes = [...new Set(trips.map(t => t.xe_ghn_cap))]
    .filter(Boolean)
    .sort((a, b) => {
      const order = ["1.25T","1T9","2.5T","3.5T","5T","7T","8T","10T","11T","12T","15T","Không rõ"];
